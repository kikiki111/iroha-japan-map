...
enum TripDetector {
    ...
    static func detect(from visits: [Visit]) -> [Trip] {
        ...
        return groups.map { group -> Trip in
            // Derive a deterministic ID from the first visit so repeated calls
            // produce stable identifiers for the same set of visits.
    ...
    ///
    /// The fixed node bytes below are arbitrary constants chosen to distinguish
    /// Iroha-generated deterministic UUIDs from random (RFC 4122 variant 2) UUIDs.
    ...
    private static func deterministicUUID(from seed: String) -> String {
        ...
        let timeHiVer = UInt16((lo & 0xFFFF) | 0x4000)     // version 4
        let clockHi   = UInt8((hi >> 8) & 0x3F) | 0x80     // variant bits
